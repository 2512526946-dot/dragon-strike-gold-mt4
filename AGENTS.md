...
所有巨龙工作流 Skill 在最终汇报末尾必须输出统一的 `【下一步操作卡】`：
...
- 操作卡只提供下一步建议和可复制指令，不代表自动授权。
- 没有下一 Skill 时，`下一 Skill` 必须写 `无`。
- 完整指令一次只能调用一个 Skill，不得连续调用多个 Skill。
- 完整指令必须要求下一轮结束时继续输出新的【下一步操作卡】。
- 不得通过操作卡自动越过用户显式批准。
- 不得通过操作卡自动 merge、tag、删除分支或进入下一工单。
- `NO-GO`、checkpoint 异常、工作区不干净或测试失败时，不得生成危险的合并、发布或开发指令。
